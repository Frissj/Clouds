...
TAG = os.environ.get("HSTR_TAG", "oct_split")
...
for pitch in (0.0, 0.5):
    for tol, name in ((0.0, "per pixel"), (10.0, "basis only"), (0.05, "default")):
        measure(f"rect {name}", dict(beamTolerance=tol, beamRefMargin=0.15, beamOct=False), 0.0, pitch)
        measure(f"oct {name}", dict(beamTolerance=tol, beamOct=True, beamOctFull=True), 0.0, pitch)
exit()
